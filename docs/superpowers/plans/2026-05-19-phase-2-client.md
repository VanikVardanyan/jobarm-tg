...
- **Infra failure inside a bot wizard is swallowed silently** (project-wide, pre-existing since Phase-1 `registerService`; surfaced in the Phase-2 final review): if Redis/DB is unreachable mid-wizard, the `conversation.external` rejection propagates only to `bot.catch` (logs, no user reply) — the user sees no feedback. Affects both `registerService` and `createRequest`. Not Phase-2-specific. Phase-6 (or a project-wide hardening pass): wrap wizard side-effects with a try/catch that replies a generic error on infra failure.
